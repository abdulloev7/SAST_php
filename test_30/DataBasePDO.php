	...
	public function sqlLog($query, $params, $dbTime, $err) {
		...
		if (empty($logSqlQuerys)) {
			...
			register_shutdown_function(function() {
				...
				\Model\SqlRedisLog::insert(
					[
						\Model\SqlRedisLog::FIELD_REQUEST => $_SERVER["REQUEST_URI"],
						\Model\SqlRedisLog::FIELD_MEMORY => memory_get_usage(true),
						\Model\SqlRedisLog::FIELD_SCRIPT_TIME => ceil(1000 * (microtime(true) - $logSessionTime)),
						\Model\SqlRedisLog::FIELD_SQL_TIME => $logSqlTime,
						\Model\SqlRedisLog::FIELD_REDIS_TIME => empty($logRedisTime) ? 0 : $logRedisTime,
						\Model\SqlRedisLog::FIELD_SQL_COUNT => count($logSqlQuerys),
						\Model\SqlRedisLog::FIELD_REDIS_COUNT => count($logRedisQuerys),
						\Model\SqlRedisLog::FIELD_SQL_QUERYS => \Encoder\Crypt::encodeObj($logSqlQuerys),
						\Model\SqlRedisLog::FIELD_REDIS_QUERYS => \Encoder\Crypt::encodeObj($logRedisQuerys),
						\Model\SqlRedisLog::FIELD_SESSION_ID => session_id(),
					]
				);
